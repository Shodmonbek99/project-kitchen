...
export default function Cart() {
  const [inputValue, setInputValue] = useState("");
  const [promo, setPromo] = useState("");
  const [cardAdded, setCardAdded] = useState(false); // Track if a card is added
  const dispatch = useDispatch();
  const cartItems = useSelector((state) => state.cart.cartItems);
  const cartTotal = useSelector((state) => state.cart.cartTotal);
  const numItemsInCart = useSelector((state) => state.cart.numItemsInCart);
  ...
  const promoCheck = () => {
    if (promo === "unwanted") {
      toast.success("You applied a valid promo code!");
      setPromo("");
      // Adjust total price after applying promo
      // For example: cartTotal = cartTotal - 5;
    } else {
  ...
  const cardAdd = () => {
    if (inputValue.length === 16) {
      ...
      setCardAdded(true); // Set cardAdded to true after successful addition
    } else {
      ...
      <div className="container mx-auto mt-16 p-4">
        ...
        <div className="flex flex-col lg:flex-row gap-10 justify-center">
          <div className="flex flex-col gap-5 w-full lg:w-2/3">
            {cartItems.length > 0 ? (
              cartItems.map((item) => (
                <div key={item.customID} className="mb-5">
                  <div className="flex flex-col lg:flex-row items-center justify-between bg-gray-500 p-3 rounded-xl">
                    <img
                      src={item.img[0]}
                      alt=""
                      className="w-32 h-32 rounded-xl"
          ...
          <div className="flex flex-col gap-5 w-full lg:w-1/3 bg-[#212121d4] p-5 rounded-2xl">
            ...
            <div className="flex flex-col gap-5 border-b-2 border-b-slate-600 pb-5">
              ...
              <input
                ...
                className={`text-white border-2 h-9 border-[#8707ff] rounded-lg p-2.5 bg-transparent focus:outline-none focus:ring-4 focus:ring-[#8707ff]/60 ${cardAdded ? 'bg-gray-200' : ''}`}
                name="text"
                type="number"
                value={inputValue}
                onChange={handleChange}
                disabled={cardAdded} // Disable input after card is added
              />
              <button onClick={cardAdd} className={`btn ${cardAdded ? 'cursor-not-allowed opacity-50' : ''}`} disabled={cardAdded}>
                <span>Add</span>
              </button>
            </div>
            <div className="flex justify-center text-center">
              <FaRegCreditCard size={70} inputValue={inputValue} />
            </div>
            <div className="flex flex-col gap-2 border-b-2 border-b-slate-600 pb-5">
              <div className="flex justify-between px-5">
                <h2 className="text-white font-bold">Product Total:</h2>
                <p className="text-emerald-500 text-xl">💲{cartTotal}</p>
              </div>
              <div className="flex justify-between px-5">
                <h2 className="text-white font-bold">Shipping:</h2>
            ...
            <div className="flex justify-between px-5">
              ...
              <p className="text-2xl text-white font-semibold">💲{cartTotal + 5}</p>
            </div>
          </div>
        </div>
      </div>
